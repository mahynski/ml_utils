...
class JensenShannonDivergence:
    ...
    * https://machinelearningmastery.com/divergence-between-probability-
    ...
    def __init__(
        ...
        robust : bool
            Whether or not to use Q[1/3] +/- 1.5*IQR as a threshold to limit the range 
            over which probability distributions are generated by histograms.
            This can make the results robust against skewed distributions with outliers
            which might cause the binning to become too coarse, but it might mask
            real differences or exclude important points if they aren't really outliers.
        """
        self.set_params(
            **{
                "epsilon": epsilon,
    ...
    def fit(self, X, y):
        ...
        def compute_(column):
            if self.robust:
                ...
                ranges = (
                        np.percentile(self.__X_[:, column], 25, interpolation = 'midpoint') - 1.5*iqr,
                        np.percentile(self.__X_[:, column], 75, interpolation = 'midpoint') + 1.5*iqr
                        )
            else:
                ranges = (
